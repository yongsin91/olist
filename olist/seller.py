...
class Seller:
    ...
    def get_review_score(self):
        ...
        reviews = self.data['order_reviews'].filter(items=['order_id','review_score'])
        items = self.data['order_items'].filter(items=['order_id','seller_id'])
        ...
        overall['share_of_five_stars'] = np.where(overall['review_score']==5,1,0)
        overall['share_of_one_stars'] = np.where(overall['review_score']==1,1,0)

        #groupby seller_id and agg to get percentage
        answer = overall.groupby(by='seller_id').agg({
            'share_of_five_stars': lambda x: round(x.sum()/x.count(),2),
            'share_of_one_stars':lambda x: round(x.sum()/x.count(),2),
